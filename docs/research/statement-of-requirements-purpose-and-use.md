...
1. **An SOR should be the governing statement of user need, not a technical design.** An SOR earns its place by making functional and non-functional outcomes, scope boundaries, constraints, and evidence of success explicit. Architecture, implementation components, product selection, delivery methods, and operating procedures explain *how* to meet those needs and must be recorded outside the SOR, including when they are mandatory elsewhere.
...
| Software or system requirements specification | Specifies system behaviour, qualities, interfaces, and constraints. | A separate technical specification derived from the SOR where needed. | It is technical and more decomposed; it is not part of the SOR. |
| Architecture or high-level design | Chooses structure and technical approach. | Responds to requirements and records design decisions. | It explains how and is not part of the SOR. |
...
- State the desired functional or non-functional outcome without technical solution information. For example, "Authorised responders shall be able to retrieve administrative activity records for the approved retention period within the agreed support window" is outcome-oriented; product, platform, architecture, configuration, and mechanism choices belong in a separate technical-design artefact.
